...
class LinearTimeInvariant(Model):
    ...
    """
    def __init__(self, data=None, A=None, DMD=True, dmdRank=None, states=None, S=None, XpXf=None):
        super().__init__(data, 'LTI', states=states)
        self.f = {}
        if data is None:  # make an empty model
            self.f['A'] = None
            self.dmd_res = {}
        elif A is not None:
            self.f['A'] = A
    ...
    def observabilityGramian(self, T=10, reduced=False):
        ...
        G = np.zeros(Ar.shape)
        for t in range(T):
#            half = self.dmd_res['u_r'] @ Ar.T @ self.dmd_res['u_r'].T @ C.T
